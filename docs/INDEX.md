# Industrial S7 Knowledge & Engineering Index
...
## 📖 Основные руководства проекта

1. **[Архитектура и протоколы (ARCHITECTURE.md)](file:///docs/ARCHITECTURE.md)** — 5-уровневая архитектура, PDU-пакетирование S7comm, классификация тегов, гибридная архивация и жизненный цикл аварий.
2. **[Руководство оператора (OPERATOR_MANUAL.md)](file:///docs/OPERATOR_MANUAL.md)** — мнемосхемы 7 переделов линии, фейсплейт механизмов, квитирование аварий, модуль трендов с пресетами и инструментом «Рука».
3. **[Руководство разработчика (DEVELOPER_GUIDE.md)](file:///docs/DEVELOPER_GUIDE.md)** — сборка, запуск, переключение хранилища `InMemory` ⇄ `Postgres`, REST API и SignalR спецификация.
4. **[Настройка ПЛК Siemens в TIA Portal (SIEMENS_SETUP.md)](file:///docs/SIEMENS_SETUP.md)** — отключение Optimized DB access, разрешение PUT/GET, импорт XML.
...
## 7. Implementation & System Modules (.NET 8/9 C#)

### 7.1. S7 Driver & PDU Batching (`src/S7Hmi.Driver.S7`)
- **`S7NetPlcDriver`**: Pure .NET S7comm driver based on `S7.Net` (MIT license) with socket resilience and RTT latency tracking.
- **`BatchRangeReader`**: Automatic PDU chunk optimizer that merges disparate tag requests within a DB into continuous memory spans, reducing round-trips by up to 20x.
- **`SimulatedPlcDriver`**: Built-in process simulator for offline demonstration and testing.

### 7.2. TIA Portal Openness (`src/S7Hmi.TiaOpenness`)
- **`TiaXmlParser`**: XML parsing of exported `<SW.Blocks.GlobalDB>` and `<SW.Tags.PlcTagTable>`.
- **`S7MemoryLayoutCalculator`**: Non-optimized DB byte and bit offset calculator applying Siemens alignment rules.

### 7.3. Database Archiver (`src/S7Hmi.Archiver.Postgres`)
- **`PostgresBatchArchiverService`**: Asynchronous batch writer utilizing `NpgsqlBinaryImporter` (`Binary COPY`) and `System.Threading.Channels` lock-free queue with circuit-breaker backoff.

### 7.4. Server & Web-HMI (`src/S7Hmi.Server` & `apps/web-hmi`)
- **`HmiHub`**: SignalR WebSockets Hub on `/hmihub` broadcasting tag delta updates and alarms.
- **Web-HMI Frontend**: React 19 + TypeScript + Tailwind CSS + uPlot 60 FPS Canvas trend charts.
