# S7 Communication Technical Research Report
...
- **Выбор ядра:** Модульный чистый TypeScript/Node.js драйвер (`plc-s7` на базе чистого асинхронного сокета TCP/RFC 1006/COTP с архитектурным заимствованием принципов `nodes7`), оформленный в виде независимого пакета, реализующего единый интерфейс `IPlcDriver`.
- **Обоснование:**
